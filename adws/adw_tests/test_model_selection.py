...
def test_model_mapping_lookups():
    ...
    test_cases = [
        # (command, model_set, expected) - All commands now use Opus 4.5
        ("/implement", "base", "opus"),
        ("/implement", "heavy", "opus"),
        ("/classify_issue", "base", "opus"),
        ("/classify_issue", "heavy", "opus"),
        ("/review", "base", "opus"),
        ("/review", "heavy", "opus"),
    ]
...
def test_get_model_for_slash_command():
    ...
    expected_base = "opus"  # All commands now use Opus 4.5
    ...
    expected_default = "opus"  # All commands now use Opus 4.5
    if model == expected_default:
        print(f"✅ With no state: /review → {model} (default to opus)")
    else:
