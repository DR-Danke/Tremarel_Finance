...
"""
Meeting transcript folder watcher that monitors External_Requirements/meeting_transcripts/
for new files.

When a new .md or .pdf transcript file is detected, triggers the ADW meeting processing
pipeline (adw_meeting_pipeline_iso.py) to process it into a CRM meeting record.

Usage:
  uv run adw_triggers/trigger_meeting_transcript_watch.py          # Continuous polling (default 30s)
  uv run adw_triggers/trigger_meeting_transcript_watch.py --once   # Single check and exit
  uv run adw_triggers/trigger_meeting_transcript_watch.py --folder /path/to/transcripts

Environment variables:
  ADW_MEETING_TRANSCRIPT_FOLDER         Override watched folder path
...
TRANSCRIPT_FOLDER = os.getenv(
    "ADW_MEETING_TRANSCRIPT_FOLDER",
    str(Path(__file__).parent.parent.parent / "External_Requirements" / "meeting_transcripts"),
)
WATCHED_EXTENSIONS = {".md", ".pdf"}
POLL_INTERVAL = int(os.getenv("ADW_MEETING_TRANSCRIPT_POLL_INTERVAL", "30"))
PROCESSED_LOG = (
    Path(__file__).parent.parent.parent / "agents" / "meeting_transcript_watch_processed.json"
)
...
def check_transcript_folder():
    """Scan the watched folder for new or modified meeting transcript files."""
    ...
    print(f"INFO: Found {len(new_files)} new meeting transcript(s):", flush=True)
    for f in new_files:
...
def trigger_pipeline(transcript_path: Path) -> bool:
    """Launch the meeting processing pipeline for a transcript file.
    ...
    """
    try:
        print(f"INFO: Triggering meeting pipeline for: {transcript_path.name}")
        pipeline_script = Path(__file__).parent.parent / "adw_meeting_pipeline_iso.py"

        if not pipeline_script.exists():
            print(f"ERROR: Meeting pipeline script not found: {pipeline_script}")
            return False

        # Create a log file for the pipeline output
        log_dir = _get_repo_root() / "agents" / "meeting_pipeline_logs"
        ...
        print(f"INFO: Meeting pipeline launched (PID {process.pid}) for {transcript_path.name}", flush=True)
        ...
        print(f"ERROR: Failed to trigger meeting pipeline for {transcript_path.name}: {e}")
        return False


def main():
    """Main entry point for the meeting transcript folder watcher."""
    global TRANSCRIPT_FOLDER

    parser = argparse.ArgumentParser(
        description="Watch a folder for new meeting transcript files and trigger the ADW meeting processing pipeline."
    )
    parser.add_argument(
        "--once",
        action="store_true",
    ...
    # Startup banner
    print("=" * 60)
    print("  ADW MEETING TRANSCRIPT FOLDER WATCHER")
    print("=" * 60)
