...
"""
ADW Test Iso (20 Attempts) - AI Developer Workflow for agentic testing in isolated worktrees

This is a modified version of adw_test_iso.py with increased retry attempts:
- Unit tests: 20 attempts (instead of 4)
- E2E tests: 10 attempts (instead of 2)

Usage:
  uv run adw_test_iso_20.py <issue-number> <adw-id> [--skip-e2e]
...
# MODIFIED: Increased from 4 to 20 for extended resolution attempts
MAX_TEST_RETRY_ATTEMPTS = 20
# MODIFIED: Increased from 2 to 10 for extended E2E resolution attempts
MAX_E2E_TEST_RETRY_ATTEMPTS = 10
...
def post_comprehensive_test_summary(
    ...
    summary = "# Comprehensive Test Results (20-Attempt Mode)\n\n"
    ...
    while attempt < max_attempts:
        ...
        make_issue_comment(
            ...
            format_issue_message(
                adw_id,
                "ops",
                f"Found {failed_count} failed tests (attempt {attempt}/{max_attempts}). Attempting resolution...",
            ),
        )
    ...
    while attempt < max_attempts:
        ...
        make_issue_comment(
            ...
            format_issue_message(
                adw_id,
                "ops",
                f"Found {failed_count} failed E2E tests (attempt {attempt}/{max_attempts}). Attempting resolution...",
            ),
        )
...
def main():
    ...
    if len(sys.argv) < 3:
        print("Usage: uv run adw_test_iso_20.py <issue-number> <adw-id> [--skip-e2e]")
        ...
        print("\nNote: This is the 20-attempt version with extended retry limits:")
        print(f"  - Unit tests: {MAX_TEST_RETRY_ATTEMPTS} attempts")
        print(f"  - E2E tests: {MAX_E2E_TEST_RETRY_ATTEMPTS} attempts")
        sys.exit(1)
    ...
    temp_logger = setup_logger(adw_id, "adw_test_iso_20")
    ...
    if state:
        ...
        make_issue_comment(
            issue_number,
            f"{adw_id}_ops: Found existing state - starting isolated testing (20-ATTEMPT MODE)\n```json\n{json.dumps(state.data, indent=2)}\n```"
        )
    else:
        # No existing state found
        logger = setup_logger(adw_id, "adw_test_iso_20")
    ...
    state.append_adw_id("adw_test_iso_20")

    # Set up logger with ADW ID from command line
    logger = setup_logger(adw_id, "adw_test_iso_20")
    logger.info(f"ADW Test Iso (20-ATTEMPT MODE) starting - ID: {adw_id}, Issue: {issue_number}, Skip E2E: {skip_e2e}")
    logger.info(f"Max unit test attempts: {MAX_TEST_RETRY_ATTEMPTS}, Max E2E attempts: {MAX_E2E_TEST_RETRY_ATTEMPTS}")
    ...
    make_issue_comment(
        issue_number,
        format_issue_message(adw_id, "ops", f"Starting isolated testing phase (20-ATTEMPT MODE)\n"
                           f"Worktree: {worktree_path}\n"
                           f"Ports - Server: {server_port}, Client: {client_port}\n"
                           f"E2E Tests: {'Skipped' if skip_e2e else 'Enabled'}\n"
                           f"Max Unit Test Attempts: {MAX_TEST_RETRY_ATTEMPTS}\n"
                           f"Max E2E Test Attempts: {MAX_E2E_TEST_RETRY_ATTEMPTS}")
    )
    ...
    if not smoke_test_response.success:
        ...
        make_issue_comment(
            ...
            format_issue_message(adw_id, "smoke_tester",
                f"Smoke test warning (non-blocking):\n```\n{smoke_test_response.output}\n```")
        )
    else:
    ...
    logger.info("Running unit tests in worktree with automatic resolution (20 attempts max)")
    make_issue_comment(
        issue_number,
        format_issue_message(adw_id, AGENT_TESTER, f"Running unit tests in isolated environment (max {MAX_TEST_RETRY_ATTEMPTS} attempts)...")
    )
    ...
    if results:
        ...
        make_issue_comment(
            issue_number,
            format_issue_message(
                adw_id, AGENT_TESTER, "No test results found in output"
            ),
        )
    ...
    e2e_failed = 0
    if not skip_e2e:
        logger.info("Running E2E tests in worktree with automatic resolution (10 attempts max)")
        make_issue_comment(
            issue_number,
            format_issue_message(adw_id, AGENT_E2E_TESTER, f"Running E2E tests in isolated environment (max {MAX_E2E_TEST_RETRY_ATTEMPTS} attempts)...")
        )
    ...
    logger.info("Isolated testing phase completed successfully (20-attempt mode)")
    make_issue_comment(
        issue_number, format_issue_message(adw_id, "ops", "Isolated testing phase completed (20-attempt mode)")
    )
    ...
    make_issue_comment(
        issue_number,
        f"{adw_id}_ops: Final test state (20-attempt mode):\n```json\n{json.dumps(state.data, indent=2)}\n```"
    )
