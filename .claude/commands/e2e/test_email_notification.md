# E2E Test: Email Notification Integration

## User Story
As a restaurant manager, I want to trigger daily task summary notifications via the API and verify that notification logs are created with the email channel, so that I can confirm the email notification pipeline works end-to-end.
...
- At least one person (type=employee) created in the restaurant with an `email` field set (e.g., "employee@example.com")
...
11. If employees with email addresses and tasks exist, verify `sent_count` >= 1.
...
17. Verify log entries include entries with `channel` equal to "email".
18. Verify email log entries have `recipient` containing "@".
19. Verify email log entries have `message` containing HTML content (e.g., "<div" or "<table").
20. Verify log entries have `status` equal to "sent" or "failed".

### Filter Notification Logs by Channel
21. Call `GET /api/notifications/log?restaurant_id={restaurant_id}&channel=email` with Authorization header.
22. Verify the response status code is 200.
23. Verify all returned entries have `channel` equal to "email".
...
- Log entries contain correct channel ("email"), recipient (valid email address), message (HTML content), and status.
...
- Console should show `INFO [EmailAdapter]: Sending email to employee@...` for each recipient.
...
- The EmailAdapter is a stub implementation; it logs sends but does not call a real SMTP server when credentials are not configured.
- Messages are formatted as HTML in Spanish (Colombian) with task descriptions and overdue indicators.
- Employees without an `email` field are sent via WhatsApp if they have a `whatsapp` field, or skipped if they have neither.
